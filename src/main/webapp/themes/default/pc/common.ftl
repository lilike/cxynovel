...
<#macro headerContent>
  ...
  <header id="global-head" <#if pageType?? && pageType==4> style="height:45px;" </#if>>
    <div>
      ...
      <div class="hd-follow" style="width:630px;text-align:right;" >
        <div class="myhide"><a href="#" class="hides">浏览记录</a>
          ...
          <span id="checklogin">
            <a href="/gotoQQLogin" rel="nofollow"><img src="/themes/default/pc/images/qq_login.gif" class="vm" alt="QQ登录"></a>
    ...
    <p class="site-logo">
        <a href="${contextPath}/" title="${labelSystemTitle}">
        <img src="${contextPath}/themes/default/pc/images/logo.png" alt="${labelSystemTitle} - logo"></a>
    </p>
    ...
    <form action="${contextPath}/search" method="get" >
       ...
       <input type="button" id="searchbuttom" value="" style="background:url('${contextPath}/themes/default/pc/images/search.jpg');border:0px solid;cursor:pointer;" />
    </form>
    </div>
    </#if>
  </header>
    ...
    <div class="channel-header-wrapper">
      <nav class="channel-nav">
        <ul class="channel-nav-list">
            <li <#if pageType ?? && pageType==1>class="current"</#if> ><a href="${contextPath}/" title="${labelSystemName}">${labelSystemName}</a></li>
            <#list categorymap?keys as categorykey>
            <li <#if category??><#if categorykey==category?c>class="current"</#if></#if>><a href="/articleList?category=${categorykey}">${categorymap[categorykey]}</a></li>
            </#list>
...
<footer id="global-foot">
    <p>
    本小说站所有小说、评论均为网友更新！仅代表发布者个人行为，与本小说站(${labelSystemUrl})立场无关！<br/>
    本站所有小说的版权为原作者所有！如无意中侵犯到您的权益，或是含有非法内容，请及时与我们联系，我们将在第一时间做出回应！谢谢！<br/>
        ${labelSystemCopyright} ${labelSystemBeianNo} ${labelSystemAnalyticscode}
